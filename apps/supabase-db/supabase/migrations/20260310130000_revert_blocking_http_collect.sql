-- Revert the blocking net.http_collect_response() call introduced in 20260310120000.
--
-- The blocking call (async := false) caused send_hourly_reminder_digests() to hang
-- indefinitely when the API was unreachable or slow, making the cron non-functional.
--
-- Root causes (Vercel Deployment Protection blocking pg_net, stale vault JWT) are now
-- fixed, so the optimistic fire-and-forget approach is correct:
--   - net.http_post() fires the request asynchronously and returns immediately.
--   - Dispatch log is written right after — same as 20260309190000.
--   - If the API rejects the request, the dispatch log row is not rolled back, but
--     the API endpoint is idempotent so duplicate calls are safe.
