-- Update handle_new_user to also create a non-deletable "myself" contact
-- for each new user. This contact mirrors the user's own identity and is
-- protected from deletion at both the API and database levels.
...
BEGIN
  ...
  -- Create a non-deletable "myself" contact that represents the user
...
-- Backfill existing users who don't already have a myself contact
INSERT INTO public.people (user_id, first_name, last_name, myself, last_interaction)
SELECT
  us.user_id,
  COALESCE(us.name, ''),
  us.surname,
