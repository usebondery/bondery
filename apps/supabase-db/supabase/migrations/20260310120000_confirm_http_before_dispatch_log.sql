-- Wait for the HTTP response from the API before writing to reminder_dispatch_log.
--
-- Before: fire-and-forget — dispatch log written immediately after net.http_post(),
--         regardless of whether the API accepted the request.
-- After:  net.http_collect_response() blocks until the response arrives, then only
--         logs and advances next_reminder_at_utc on a 2xx. Non-2xx exits without
--         touching the log so the cron retries on the next cycle.
...
DECLARE
  ...
  api_secret text;
  request_id bigint;
  http_result net.http_response_result;
...
BEGIN
  ...
  -- Block until the HTTP response arrives so we only log confirmed deliveries.
  http_result := net.http_collect_response(request_id, async := false);
  http_status_code := (http_result.response).status_code;

  IF http_status_code NOT BETWEEN 200 AND 299 THEN
    RETURN jsonb_build_object(
      'success', false,
      'scheduledUsers', user_count,
      'httpStatus', http_status_code,
      'targetUrl', api_url,
      'message', 'API returned non-2xx — will retry next cron cycle'
    );
  END IF;
