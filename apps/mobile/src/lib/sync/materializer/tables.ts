...
export function applyPeopleAddressesShapeOp(db: SQLiteDatabase, message: ShapeRowMessage): void {
  applyGenericShapeOp(
    ...
    defaultColumns([
      ...
      { key: "sort_order", read: (r) => readNumber(r, "sort_order") ?? 0 },
      {
        key: "address_granularity",
        read: (r) => readString(r, "address_granularity") ?? "unknown",
      },
...
export function applyPeopleGroupsShapeOp(db: SQLiteDatabase, message: ShapeRowMessage): void {
  applyGenericShapeOp(
    db,
    "people_groups",
    defaultColumns(["id", "person_id", "group_id", "user_id", "created_at"]),
    message,
  );
}
...
export function applyPeopleTagsShapeOp(db: SQLiteDatabase, message: ShapeRowMessage): void {
  applyGenericShapeOp(
    db,
    "people_tags",
    defaultColumns(["id", "person_id", "tag_id", "user_id", "created_at"]),
    message,
  );
}

export function applyPeopleImportantDatesShapeOp(
  db: SQLiteDatabase,
  message: ShapeRowMessage,
): void {
  applyGenericShapeOp(
