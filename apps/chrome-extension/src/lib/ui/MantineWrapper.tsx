...
export function MantineWrapper({
  ...
  useEffect(() => {
    ...
    async function loadPreference() {
      try {
        ...
        extLog.error("[theme] Failed to load theme preference:", error);
      }
    }
  ...
  const setThemePreference = useCallback(async (value: "auto" | "light" | "dark") => {
    setThemePreferenceState(value);

    try {
      await browser.storage.local.set({ [THEME_STORAGE_KEY]: value });
    } catch (error) {
      extLog.error("[theme] Failed to save theme preference:", error);
    }
  }, []);
