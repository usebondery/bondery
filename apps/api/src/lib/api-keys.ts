...
let cachedSigningMaterial: { jwkJson: string; material: JwtSigningMaterial } | null =
  null;
...
export async function loadJwtSigningMaterial(
  jwkJson: string,
): Promise<JwtSigningMaterial> {
  ...
  if (!jwk.d) {
    throw new Error(
      "PRIVATE_SUPABASE_JWT_SIGNING_JWK must be a private key (include d)",
    );
  }
  ...
  if (!(importedKey instanceof CryptoKey)) {
    throw new Error(
      "PRIVATE_SUPABASE_JWT_SIGNING_JWK must be an ECDSA private key",
    );
  }
