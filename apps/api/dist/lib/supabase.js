...
export function createAdminClient() {
    ...
    return createClient(PUBLIC_SUPABASE_URL, PRIVATE_SUPABASE_SECRET_KEY);
}
/**
 ...
 */
function combineChunkedCookies(cookies) {
    const combined = {};
    const chunks = {};
...
function getAuthTokensFromCookies(request) {
    ...
    for (const [key, value] of Object.entries(cookies)) {
        ...
        if (value) {
            ...
            try {
                ...
                console.log("[getAuthTokensFromCookies] Failed to parse as JSON:", e instanceof Error ? e.message : String(e));
...
export async function createAuthenticatedClient(request) {
    ...
    const client = createClient(PUBLIC_SUPABASE_URL, PUBLIC_SUPABASE_PUBLISHABLE_KEY, {
        auth: {
