...
export async function redirectRoutes(fastify) {
    ...
    fastify.post("/", async (request, reply) => {
        ...
        const { instagram, linkedin, facebook, firstName, middleName, lastName, profileImageUrl, title, place, } = request.body;
        ...
        if (instagram)
            insertData.instagram = instagram;
        if (linkedin)
            insertData.linkedin = linkedin;
        if (facebook)
            insertData.facebook = facebook;
        if (middleName)
            insertData.middle_name = middleName;
        if (lastName)
    ...
    fastify.get("/", async (request, reply) => {
        ...
        const { instagram, linkedin, facebook, firstName, middleName, lastName, profileImageUrl, title, place, } = query;
        if (!instagram && !linkedin && !facebook) {
            return reply.status(400).send({
                error: "Instagram, LinkedIn, or Facebook username is required",
            });
        ...
        if (!user) {
            ...
            return reply.redirect(`${URLS.webapp}${URLS.login}?returnUrl=${encodeURIComponent(returnUrl)}`);
        }
        // Build query to look up contact
 ...
 */
async function updateContactPhoto(supabase, contactId, userId, imageUrl) {
    try {
