...
Current auxiliary segments: `merge`, `merge-recommendations`, `enrich-queue`, `keep-in-touch`.
...
Section order in GitBook follows `OPENAPI_TAG_ORDER` in `packages/schemas/src/openapi/route-order.ts` (applied via `apps/api/src/openapi/swagger-config.ts`). Order by **integrator dependency**, not alphabet:
...
Top-level route mount order: `apps/api/src/routes/register-all.ts` (`ROUTE_MOUNTS`). Route areas (`integration`, `session`, `admin`, `internal`): `apps/api/src/lib/platform/route-areas.ts`.
...
`apps/api/scripts/check-openapi-route-order.ts` validates tier and HTTP method order in the generated OpenAPI spec (tier must not decrease within a tag; same path must follow GET → POST → PUT → PATCH → DELETE). See also `docs/contributing/api-routes.mdx`.
