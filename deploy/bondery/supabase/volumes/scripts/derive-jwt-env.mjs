 ...
 * Used by the jwt-derive compose init service.
 *
 * Usage:
 *   node derive-jwt-env.mjs /path/to/jwt-derived.env
...
function formatEnvFile(derived) {
  return [
    "# Generated by jwt-derive — do not edit. Re-run: docker compose run --rm jwt-derive",
    envFileLine("GOTRUE_JWT_KEYS", derived.GOTRUE_JWT_KEYS),
    envFileLine("JWT_JWKS", derived.JWT_JWKS),
    envFileLine("PGRST_JWT_SECRET", derived.PGRST_JWT_SECRET),
    envFileLine("API_JWT_JWKS", derived.API_JWT_JWKS),
    envFileLine("ANON_KEY_ASYMMETRIC", derived.ANON_KEY_ASYMMETRIC),
    envFileLine("SERVICE_ROLE_KEY_ASYMMETRIC", derived.SERVICE_ROLE_KEY_ASYMMETRIC),
    "",
  ].join("\n");
}
